...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    for j in $(seq $nj_full); do
      Args[$j]=`echo "ivector-extractor-acc-stats --binary='false' --num-threads=$num_threads --num-samples-for-weights=$num_samples_for_weights $dir/$x.ie '$feats' 'ark,s,cs:gunzip -c $dir/post.JOB.gz|' -|" | sed s/JOB/$j/g`
      Args[$j]=`echo "ivector-extractor-acc-stats --binary='false' --num-threads=$num_threads --num-samples-for-weights=$num_samples_for_weights $dir/$x.ie '$feats' 'ark,s,cs:gunzip -c $dir/post.JOB.gz|' -|" | sed s/JOB/$j/g`
    done
